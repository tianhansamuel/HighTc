...
def GB(omega,px,py,t,t1,t2,t3,t4,ww):
   GGB=1.0/(omega**2-(tb(px,py,t,t1,t2,t3,t4)+Self(omega,px,py,t,t1,t2,t3,t4))**2-0.5*ww**2*(cos(px)-cos(py))**2+0.01*1j) 
   return GG
...
def DE(px,py,t,t1,t2,t3,t4,nd):
    omega2=tb(px+pi,py+pi,t,t1,t2,t3,t4)-tb(px,py,t,t1,t2,t3,t4)+AMAX(nd)[0]
    return omega2
  ...
  for ix in range(NN):
   for iy in range(NN):
     ...
     if abs(tb(kx,ky,t,t1,t2,t3,t4)-EF)<0.02:
       NFG+=1
       GR+=GG(omega,kx,ky,t,t1,t2,t3,t4)
  GR=GR/NFG
  return GR
TEMP=0.001
...
for ii in range(NN):
    ...
    G1=gn(omega1-0.04,-0.04,-0.5908,0.0962,-0.1306,-0.0507,0.0939).imag*1.0/(exp(1.0*(omega1-0.04)/TEMP)+1)
    G2=gn(omega1-0.06,-0.06,-0.5908,0.0962,-0.1306,-0.0507,0.0939).imag*1.0/(exp(1.0*(omega1-0.06)/TEMP)+1)
    G3=gn(omega1-0.08,-0.08,-0.5908,0.0962,-0.1306,-0.0507,0.0939).imag*1.0/(exp(1.0*(omega1-0.08)/TEMP)+1)
    G4=gn(omega1-0.1,-0.1,-0.5908,0.0962,-0.1306,-0.0507,0.0939).imag*1.0/(exp(1.0*(omega1-0.1)/TEMP)+1)
    G8=gn(omega1-0.11,-0.11,-0.5908,0.0962,-0.1306,-0.0507,0.0939).imag*1.0/(exp(1.0*(omega1-0.12)/TEMP)+1)
    G5=gn(omega1-0.12,-0.12,-0.5908,0.0962,-0.1306,-0.0507,0.0939).imag*1.0/(exp(1.0*(omega1-0.12)/TEMP)+1)
    G6=gn(omega1-0.14,-0.14,-0.5908,0.0962,-0.1306,-0.0507,0.0939).imag*1.0/(exp(1.0*(omega1-0.14)/TEMP)+1)
    G7=gn(omega1-0.16,-0.16,-0.5908,0.0962,-0.1306,-0.0507,0.0939).imag*1.0/(exp(1.0*(omega1-0.16)/TEMP)+1)
