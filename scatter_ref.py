...
N1=100
NB=100
ff1=open('scatter1_ref_0_re.dat','w')
ff2=open('scatter1_ref_0_imag.dat','w')
...
e1=tb(pi,0.27*pi,-0.5908,0.0962,-0.1306,-0.0507,0.0939)
...
for ix in range(NN):
 ...
 for iy in range(NN):
   ...
   if tb(kx,ky,-0.5908,0.0962,-0.1306,-0.0507,0.0939)-tb(kx+pi,ky+pi,-0.5908,0.0962,-0.1306,-0.0507,0.0939)>0.04:
    TT=GG(e1+ed+0.1,kx,ky,-0.5908,0.0962,-0.1306,-0.0507,0.0939,0.07,0.04,0.14).real
    TT1=GG(e1+ed+0.1,kx,ky,-0.5908,0.0962,-0.1306,-0.0507,0.0939,0.07,0.04,0.14).imag
   else:
...
### 0. 0.32pi  2. 0.30pi 1. 0.27pi 3. 0.25pi
