...
def GB(omega,px,py,t,t1,t2,t3,t4,DD):
   GGB=(omega+tb(px,py,t,t1,t2,t3,t4)+Self(omega,px,py,t,t1,t2,t3,t4)+0.001*1j)/((omega+0.001*1j)**2-(tb(px,py,t,t1,t2,t3,t4)+Self(omega,px,py,t,t1,t2,t3,t4))**2-0.5*DD**2*(cos(px)-cos(py))**2) 
   return GGB
...
def AMAX(dd):
    EG=100
    aa=0/(exp(1.0*(omega1-0.04)/TEMP)+1)
    for ia in range(NN):
       alpha=0.3*ia/NN
       if om(alpha,dd)<EG:
          aa=alpha
  ...
  for ix in range(NN):
   for iy in range(NN):
     ...
     if abs(tb(kx,ky,t,t1,t2,t3,t4)-EF)<0.02:
       NFG+=1
       GR+=GB(omega,kx,ky,t,t1,t2,t3,t4,0.04)
  GR=GR/NFG
  return GR
TEMP=0.001
...
for ii in range(NB):
    ...
    G9=gn(omega1-0.18,-0.18,-0.5908,0.0962,-0.1306,-0.0507,0.0939).imag
