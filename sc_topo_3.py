...
def GAMMA(kx,ky,T,t,t1,t2,t3,t4,aa1,ww,dop):
    ...
    if ww**2-(xi1-xi)**2!=0:
     SS=0.5*(cos(kx)-cos(ky))**2*(1-12*aa1**2)*pi/(1+12*aa1**2)*xi1**2*ww/(ww**2-(xi1-xi)**2)*(TANH(ww,T)-TANH(xi1-xi,T))
    else: SS=-0.5*(cos(kx)-cos(ky))**2*(1-12*aa1**2)*pi/(1+12*aa1**2)*xi1**2*ww/(ww+(xi1-xi))*1.0/(DF*(xi1-xi))
    return SS
...
def TCD(EF,t,t1,t2,t3,t4,ww,aa,dop):
    ...
    for tt1 in range(NN1):
       TTC=0.1-0.1*tt1/NN1
       dt=abs(abs(CCT(EF,TTC,t,t1,t2,t3,t4,ww,aa,dop))-1)
       if dt<dd:
         tc=TTC
         dd=dt
    return tc
